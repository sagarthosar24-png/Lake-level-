...
if st.button("Calculate Final Levels", type="primary"):
    try:
        # 1. Initial Data Fetch (Find Volume from RL)
        ...
        # 2. Generation Impacts (Subtract for Upper, Subtract for Lower PH Discharge)
        u_running_mcm = u_start_mcm - (gen_mus_in * U_PH_CONV)
        l_running_mcm = l_start_mcm - (l_gen_mus_in * L_PH_CONV)
        
        # 3. Iterative Gate Transfer Loop
        gate_vol_total = 0.0
        if gate_is_open and hours_open > 0:
            total_min = int(hours_open * 60)
            for m in range(0, total_min):
                # Clamp to ensure volumes stay within table range
                u_running_mcm = np.clip(u_running_mcm, u_content_data.min(), u_content_data.max())
                l_running_mcm = np.clip(l_running_mcm, l_content_data.min(), l_content_data.max())

                # Find Current RLs for head difference
                curr_u_idx = (np.abs(u_content_data - u_running_mcm)).argmin()
                curr_l_idx = (np.abs(l_content_data - l_running_mcm)).argmin()
                
                head_diff = u_level_data[curr_u_idx] - l_level_data[curr_l_idx]
                
                if head_diff <= 0:
                    break # Stop if levels equalized
        ...
        # 4. Final RL Lookups (Find RL from Volume)
        final_u_idx = (np.abs(u_content_data - u_running_mcm)).argmin()
        final_u_rl = u_level_data[final_u_idx]
        
        final_l_idx = (np.abs(l_content_data - l_running_mcm)).argmin()
        final_l_rl = l_level_data[final_l_idx]
